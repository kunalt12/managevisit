    ...
    angular.module("BlurAdmin").factory("SettingsFactory", ["$http", "$q", "$config", function ($http, $q, $config) {
        ...
        SettingsFactory.validation = {
            admin_email: {
                ...
                minlength: "Admin email is too short",
                maxlength: "Admin email is too long"
            },
            confirm_link_expired: {
                required: "Hours is required",
                minlength: "Hours is too short",
                maxlength: "Hours is too long"
            }
        };
        SettingsFactory.getAllSettings = function () {
            return $http.get($config.api_url + "settings");
        };
        SettingsFactory.updateRecord = function (t, o) {
            return $http.put($config.api_url + "settings/" + t, o);
        };
